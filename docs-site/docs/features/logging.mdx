...
Force a rotation immediately with:

```bash
prisma-airs-hooks rotate-log
```

## Reviewing Recent Entries

Tail the most recent scan log entries without opening the raw file:

```bash
# last 10 entries (default)
prisma-airs-hooks logs

# last 20 entries
prisma-airs-hooks logs --n 20

# raw JSON Lines, e.g. for piping into jq
prisma-airs-hooks logs --n 50 --json
```
