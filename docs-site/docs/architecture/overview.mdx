...
flowchart TB
    subgraph Cursor IDE
        ...
        C -.-> S1[Shell Command]
        S1 -.-> S2[beforeShellExecution*]
        S2 -.->|allow| S3[Execute] -.-> S4[afterShellExecution*]
        C -.-> R1[File Read] -.-> R2[beforeReadFile* / beforeTabFileRead*]
        C -.-> G1[Subagent Task] -.-> G2[subagentStart*]
    end
...
The `install-hooks` script points at compiled JS by default. See [Contributing](../development/contributing) for the development workflow.

### Optional Hook Scans

The [optional hooks](../features/optional-hooks) reuse the same lifecycle with different content routing:
