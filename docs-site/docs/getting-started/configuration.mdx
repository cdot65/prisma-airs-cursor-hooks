...
`sanitize_mcp_output` (default `false`): in `enforce` mode, `postToolUse` replaces flagged MCP tool output with a redaction notice before it reaches the model. See [Optional Hooks](../features/optional-hooks#mcp-output-sanitization-posttooluse).
